...
class Solver_2opt:
    ...
    def solve(self):
        tour, path_length = Solver_greedy(self.cities).solve()
        updated = True
        while updated:
            updated = False
            for i in range(len(tour) - 2):
                for j in range(i + 2, len(tour) - 1):
                    ...
                    if (((d1 > 0 and d2 < 0) or (d1 < 0 and d2 > 0)) and
                        ...
                        path_length -= distance(c1, c2)
                        path_length -= distance(c3, c4)
                        path_length += distance(c1, c3)
                        path_length += distance(c2, c4)
                        tour = (tour[: i + 1] +
